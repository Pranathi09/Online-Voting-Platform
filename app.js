...
app.post("/admin", async (request, response) => {
  ...
  if (request.body.password.length <= 7) {
    request.flash("error", "Password length should be minimum of length 8!");
    return response.redirect("/signup");
  }
  ...
  try {
    ...
    console.log(error);
    request.flash("error", "Existing User!!!");
    return response.redirect("/signup");
  }
});
...
app.post(
  ...
  async (request, response) => {
    ...
    try {
      ...
      console.log(error);
      request.flash("error", "URL is already in use, try another one!!!");
      return response.redirect("/electionpage/addelection");
    }
  }
);
